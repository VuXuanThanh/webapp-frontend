<template>
  <div class="col l-3">
    <router-link :to="'/product/'+product.productId" class="home-product-item" href="#">
      ...
      <div class="home-product-item__img" :style="{backgroundImage: `url(${getImageAvartar})`}"></div>
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      getImageAvartar() {
        let me = this;
        let image = me.ProductImages.filter(
          (image) => image.productId == me.product.productId
        );
    ...
    methods: {
      ...
      formatPrice(price) {
        let value =
          price === "orgin" ? this.product.priceOrgin : this.product.priceDeal;
